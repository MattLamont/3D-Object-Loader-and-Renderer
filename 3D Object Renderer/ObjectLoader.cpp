...
ObjectLoader::ObjectLoader( OpenGLFrame *gl )
{
	openGLFrame = gl;
	currentObject = openGLFrame->currentObject;
}


void ObjectLoader::setObjectFilename( QString f )
{
	std::string s = f.toStdString();
	filename = s.c_str();
	Load3DS( currentObject );
	openGLFrame->setDrawableObject( OpenGLFrame::OBJECT::FILE_OBJECT );
}

char ObjectLoader::Load3DS( OpenGLFrame::obj_type_ptr p_object )
{
